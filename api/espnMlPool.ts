...
import { hfaFor, fairMoneylineFromWinPct, spreadToWinPct } from "../odds";
...
export async function fetchEspnMlPicksForWeek(
  ...
  return picks.map((p) => {
    ...
    const awayWinProb = myProjAwaySpread != null ? spreadToWinPct(myProjAwaySpread) : null;
    const myProjAwayMoneyline = fairMoneylineFromWinPct(awayWinProb);
    const myProjHomeMoneyline = awayWinProb != null ? fairMoneylineFromWinPct(1 - awayWinProb) : null;
