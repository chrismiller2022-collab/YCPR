  ...
  try {
    if (action === "selectGames") {
      ...
      if (toDelete.length > 0) {
        const { error: deleteError } = await supabaseAdmin.from("espn_confidence_picks").delete().in("id", toDelete);
    ...
    if (action === "savePicks") {
      ...
      // Guard against duplicate confidence point values within this save —
      // the UI already prevents this by excluding used values from other
      // rows' dropdowns, but this is a second check in case of a stale
      // client or a direct API call.
      const usedPoints = picks
        .map((p: any) => p.confidence_points)
        .filter((v: any) => v != null);
      const seen = new Set<number>();
      for (const v of usedPoints) {
        if (seen.has(v)) {
          res.status(400).json({ error: `Confidence value ${v} is assigned to more than one game` });
          return;
        }
        seen.add(v);
      }
      ...
      for (const p of picks) {
        const { error } = await supabaseAdmin
          ...
          .update({
            ...
            confidence_points: p.confidence_points ?? null,
