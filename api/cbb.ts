...
// College Basketball's own consolidated endpoint — separate from
// api/ratings.ts (which is CFB-only) so the two sports never share code
// that could accidentally cross-contaminate conventions (CBB ratings
// run higher-is-better on their own native scale; there's no CBB
// equivalent of this site's negative-is-better CFB convention, and no
// reason to force one). Same action-dispatched, password-gated pattern
// as ratings.ts, and for the same reason: Vercel Hobby's 12-function cap.
