  ...
  function load() {
    ...
    fetchEspnSpreadPicksForWeek(season, week, liveByTeam)
      .then((data) => {
        ...
        for (const p of data) {
          // Default to whichever side covers per my model vs ESPN's own
          // line, if nothing's been picked yet — e.g. my spread +3,
          // ESPN's +5: my model thinks the favorite wins by less than
          // ESPN's cushion, so the underdog covers. Chris can still
          // change it before submitting.
          const autoPick: "away" | "home" | null =
            p.picked_side ??
            (p.myProjAwaySpread == null || p.espnAwaySpread == null
              ? null
